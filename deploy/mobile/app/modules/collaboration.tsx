...
export default function CollaborationScreen() {
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{ title: 'Collaboration', headerRight: () => (
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.statsRow}>
          <View style={styles.statCard}><MaterialCommunityIcons name="message-text" size={24} color="#7c3aed" /><Text style={styles.statValue}>{discussions.length}</Text><Text style={styles.statLabel}>Discussions</Text></View>
          <View style={styles.statCard}><MaterialCommunityIcons name="comment-multiple" size={24} color="#3b82f6" /><Text style={styles.statValue}>{discussions.reduce((sum, d) => sum + (d.repliesCount || 0), 0)}</Text><Text style={styles.statLabel}>Replies</Text></View>
        </View>

        <Text style={styles.sectionTitle}>Recent Discussions</Text>
        {discussions.slice(0, 20).map((disc, i) => (
          <Pressable key={disc.id || i} style={styles.card} onPress={() => setSelectedDiscussion(disc)}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardTitle}>{disc.title}</Text>
            ...
            <Text style={styles.cardSub} numberOfLines={2}>{disc.content}</Text>
            <View style={styles.cardFooter}>
              <Text style={styles.cardMeta}><MaterialCommunityIcons name="account" size={12} color="#64748b" /> {disc.author || 'Anonymous'}</Text>
              <Text style={styles.cardMeta}><MaterialCommunityIcons name="comment" size={12} color="#64748b" /> {disc.repliesCount || 0} replies</Text>
            </View>
          </Pressable>
        ))}
        {discussions.length === 0 && <Text style={styles.empty}>No discussions yet. Start the conversation!</Text>}
      </ScrollView>
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={[styles.input, {height: 100}]} placeholder="Content" multiline value={formData.content} onChangeText={t => setFormData({...formData, content: t})} />
            <Text style={styles.label}>Category</Text>
            <View style={styles.chipRow}>
              {CATEGORIES.map(c => (
                <Pressable key={c} style={[styles.chip, formData.category === c && styles.chipActive]} onPress={() => setFormData({...formData, category: c})}>
      ...
      <Modal visible={!!selectedDiscussion} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{selectedDiscussion?.title}</Text>
            ...
            <Text style={styles.detailValue}>{selectedDiscussion?.author || 'Anonymous'}</Text>
            <Text style={styles.detailLabel}>Content</Text>
            <Text style={styles.detailValue}>{selectedDiscussion?.content || 'No content'}</Text>
            <Text style={styles.detailLabel}>Posted</Text>
            <Text style={styles.detailValue}>{selectedDiscussion?.createdAt ? new Date(selectedDiscussion.createdAt).toLocaleDateString() : 'N/A'}</Text>
            <Text style={styles.detailLabel}>Replies</Text>
            <Text style={styles.detailValue}>{selectedDiscussion?.repliesCount || 0} replies</Text>
            <Pressable style={styles.closeBtn} onPress={() => setSelectedDiscussion(null)}><Text style={styles.closeText}>Close</Text></Pressable>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: '#fff', borderRadius: 12, padding: 16, alignItems: 'center', marginHorizontal: 4 },
  ...
  chip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, backgroundColor: '#f1f5f9', marginRight: 8, marginBottom: 8 },
