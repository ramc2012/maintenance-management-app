...
export default function MRPScreen() {
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{ title: 'Material Planner' }} />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.summaryCard}>
          <Text style={styles.summaryTitle}>Budget Overview</Text>
          <View style={styles.summaryRow}>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryValue}>{fmt(totalAllocated)}</Text>
        ...
        <View style={styles.catGrid}>
          {CATEGORIES.map(c => (
            <Pressable key={c.id} style={[styles.catCard, {backgroundColor: c.bg}, categoryFilter === c.id && styles.catActive]} onPress={() => setCategoryFilter(categoryFilter === c.id ? null : c.id)}>
              ...
              <Text style={styles.catLabel}>{c.id}</Text>
              <Text style={[styles.catValue, {color: c.color}]}>{fmt(totals[c.id]?.allocated || 0)}</Text>
              <Text style={styles.catMeta}>{totals[c.id]?.allocated > 0 ? Math.round((totals[c.id].utilized/totals[c.id].allocated)*100) : 0}% used</Text>
            </Pressable>
          ))}
        </View>
        ...
        <View style={styles.searchBox}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search budgets..." value={filter} onChangeText={setFilter} />
        </View>

        <Text style={styles.sectionTitle}>Budget Items ({filteredBudgets.length})</Text>
        {filteredBudgets.slice(0, 20).map((b, i) => (
          <Pressable key={b.id || i} style={styles.card} onPress={() => setSelectedBudget(b)}>
            ...
            <View style={styles.cardRow}>
              <Text style={styles.cardMeta}>Allocated: {fmt(b.allocatedAmount || 0)}</Text>
              <Text style={styles.cardMeta}>Used: {fmt(b.utilizedAmount || 0)}</Text>
            </View>
        ...
        {filteredBudgets.length === 0 && <Text style={styles.empty}>No budgets found</Text>}
      </ScrollView>
      ...
      <Modal visible={!!selectedBudget} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{selectedBudget?.title}</Text>
            <Text style={styles.detailLabel}>Category</Text>
            <Text style={styles.detailValue}>{selectedBudget?.category}</Text>
            <Text style={styles.detailLabel}>Allocated Amount</Text>
            <Text style={styles.detailValue}>{fmt(selectedBudget?.allocatedAmount || 0)}</Text>
            <Text style={styles.detailLabel}>Utilized Amount</Text>
            <Text style={styles.detailValue}>{fmt(selectedBudget?.utilizedAmount || 0)}</Text>
            <Text style={styles.detailLabel}>Remaining</Text>
            <Text style={styles.detailValue}>{fmt((selectedBudget?.allocatedAmount || 0) - (selectedBudget?.utilizedAmount || 0))}</Text>
            <Text style={styles.detailLabel}>Fiscal Year</Text>
            <Text style={styles.detailValue}>{selectedBudget?.fiscalYear || 'N/A'}</Text>
            <Pressable style={styles.closeBtn} onPress={() => setSelectedBudget(null)}><Text style={styles.closeText}>Close</Text></Pressable>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  catLabel: { fontSize: 10, fontWeight: '600', color: '#475569', marginTop: 8 },
  catValue: { fontSize: 14, fontWeight: '700', marginTop: 4 },
  catMeta: { fontSize: 10, color: '#94a3b8', marginTop: 2 },
