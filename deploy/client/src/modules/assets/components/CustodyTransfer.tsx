...
export const CustodyTransfer = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [mRes, iRes, pRes, mtRes, instRes, typeRes] = await Promise.all([
                axios.get('/api/equipment/custody'),
    ...
    const handleSubmit = async (values: any) => {
        try {
            ...
            if (editMode && selectedMeter) {
                await axios.put(`/api/equipment/custody/${selectedMeter.id}`, values);
                message.success('Meter updated');
            } else {
                await axios.post('/api/equipment/custody', values);
                message.success('Meter created');
            }
            setOpen(false);
            setEditMode(false);
    ...
    const handleAddInstrument = async (values: any) => {
        try {
            // Automatically link to current meter
            values.custodyMeterId = selectedMeter.id;
    ...
    const openDetails = (meter: any) => {
        setSelectedMeter(meter);
        setDetailsOpen(true);
    };

    const meterInstruments = instruments.filter((i:any) => i.custodyMeterId === selectedMeter?.id);
    ...
    const columns = [
        { title: 'Meter ID', dataIndex: 'meterId', render: (t:string, r:any) => (
            <span className="font-mono font-bold text-blue-600 cursor-pointer" onClick={() => openDetails(r)}>{t}</span>
        )},
        ...
        { title: 'Instruments', render: (_:any, r:any) => {
            const count = instruments.filter((i:any) => i.custodyMeterId === r.id).length;
    ...
    return (
        <div className="p-4 space-y-4">
            <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow border dark:border-gray-700">
                <h2 className="text-lg font-bold dark:text-white">Custody Transfer Meters</h2>
            ...
            <Modal title={editMode ? "Edit Meter" : "Add Custody Meter"} open={open} onCancel={() => setOpen(false)} onOk={form.submit} width={650}>
                <Form form={form} layout="vertical" onFinish={handleSubmit}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Form.Item name="customerId" label="Customer ID" rules={[{required: true}]}><Input/></Form.Item>
                    ...
                    <Form.Item name="details" label="Notes"><Input.TextArea rows={2}/></Form.Item>
                </Form>
            </Modal>
                ...
                {selectedMeter && (
                    <div className="space-y-4">
                        <Descriptions bordered size="small" column={2}>
                            <Descriptions.Item label="Customer">{selectedMeter.customerName}</Descriptions.Item>
                            ...
                            <Descriptions.Item label="Pipe Size">{selectedMeter.pipeSize ? selectedMeter.pipeSize + " inch" : "-"}</Descriptions.Item>
                            <Descriptions.Item label="Element Size">{selectedMeter.elementSize ? selectedMeter.elementSize + " inch" : "-"}</Descriptions.Item>
                        </Descriptions>
                        
                        {selectedMeter.configData && Object.keys(selectedMeter.configData).length > 0 && (
                            <Card size="small" title="Type Configuration">
                                <Descriptions size="small" column={2}>
                                    {Object.entries(selectedMeter.configData).map(([k, v]) => (
