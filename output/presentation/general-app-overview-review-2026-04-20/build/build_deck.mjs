...
const DECK_ID = "general-app-overview-review";
const OUT_DIR = "/Users/chinnadurairamachandran/Downloads/maintenance management app/output/presentation/general-app-overview-review-2026-04-20/final";
const REF_DIR = "/Users/chinnadurairamachandran/Downloads/maintenance management app/output/presentation/general-app-overview-review-2026-04-20/references";
...
const SOURCES = {
  ...
  clientPkg: "deploy/client/package.json",
  serverPkg: "deploy/server/package.json",
...
const SLIDES = [
  {
    kicker: "GENERAL APP OVERVIEW",
    title: "Maintenance Management Platform\nFeatures, Structure & Review",
    subtitle:
      "General engineering deck covering app features, product structure, tech stack and expert code review for the ONGC Ankleshwar Asset maintenance system.",
    moment: "Broad maintenance platform across web, mobile and AI",
    notes:
      "Open with a general framing: this is an enterprise maintenance platform with multiple user surfaces and one operational backbone.",
    sources: ["readme", "app", "hub", "server", "schema", "mobilePkg"],
  },
  {
    kicker: "FEATURE LANDSCAPE",
    title: "The app already covers a wide operational feature set",
    subtitle:
      "The current product footprint spans day-to-day maintenance execution, planning workflows, document access, analytics and cognitive assistance.",
    metrics: [
      ["18", "Role-aware module entries on the web hub", "EnterpriseHub.tsx"],
      ["31", "Mounted API route groups behind the app", "server/src/index.ts"],
      ["90", "Prisma models in the shared operational schema", "server/prisma/schema.prisma"],
    ],
    badges: ["Execution", "Planning", "Knowledge"],
    notes:
      "Use the numbers as evidence of scope, then talk about the product categories: execution modules, planning modules and knowledge modules.",
    sources: ["hub", "server", "schema"],
  },
  {
    kicker: "PRODUCT STRUCTURE",
    title: "The product is structured as linked user surfaces, not one giant page",
    subtitle:
      "The experience begins with a universal login, branches into specialist modules, and extends into mobile and Kelvin AI without creating a separate data universe.",
    cards: [
      ["Web console", "The browser app acts as the control-room shell: role-filtered hub, specialist module routes, dense workflows and a floating Kelvin assistant."],
      ["Mobile shell", "The Expo app mirrors field work with KPI strip, health watch, module cards and secure token-backed access for technicians and supervisors."],
      ["Shared identity", "Users authenticate once and move across modules instead of encountering isolated module logins or disconnected experiences."],
    ],
    badges: ["Web", "Mobile", "Universal Login"],
    notes:
      "This slide should feel product-oriented rather than purely technical. Explain the three user surfaces and the importance of one session model.",
    sources: ["app", "hub", "mobileHome", "chat"],
  },
  {
    kicker: "TECH STACK",
    title: "The stack is modern and split cleanly by responsibility",
    subtitle:
      "Frontend, application, database and cognitive concerns are separated clearly enough that each tier can evolve without turning the system into a monolith.",
    cards: [
      ["Frontend stack", "Web uses React with Vite, React Router, Ant Design and Tailwind. Mobile uses Expo Router, React Native 0.81.5 and React 19.1."],
      ["Backend stack", "Server uses Express, Prisma, JWT auth, cron services and module-oriented controllers over PostgreSQL-backed operational data."],
      ["AI + runtime stack", "Kelvin runs on FastAPI with RAG hooks, local Ollama integration and Gemma 4 model discovery, while Docker composes the local platform."],
    ],
    badges: ["React + Vite", "Express + Prisma", "FastAPI + Ollama"],
    notes:
      "Keep this slide general and stack-focused. Call out exact technologies only where they help position maturity and maintainability.",
    sources: ["clientPkg", "serverPkg", "mobilePkg", "cognitiveMain", "llm", "readme"],
  },
  {
    kicker: "SYSTEM ARCHITECTURE",
    title: "The architecture separates user experience, orchestration and intelligence cleanly",
    subtitle:
      "The app behaves like one platform because routing, auth and data contracts remain centralized even though the runtimes are separated.",
    cards: [
      ["Experience layer", "Web and mobile provide task-specific shells for planners, engineers, supervisors and technicians while preserving shared navigation and auth logic."],
      ["Service layer", "Express route groups connect the UI to module controllers for procurement, work orders, manuals, KPIs, inspections, contracts and more."],
      ["Data layer", "Prisma maps a large shared schema so modules can read and write the same operational records instead of fragmenting the plant data model."],
    ],
    badges: ["Experience", "Services", "Shared Data"],
    notes:
      "Architecturally, the key message is controlled separation with a shared contract. The modules are many, but the platform model is singular.",
    sources: ["readme", "server", "schema", "cognitiveMain"],
  },
  {
    kicker: "CODE STRUCTURE",
    title: "The codebase is organized by product domain first, framework second",
    subtitle:
      "That structure makes ownership understandable and supports modular growth, but it also exposes where refactoring and stronger testing discipline are needed.",
    metrics: [
      ["29", "Web routes declared in the shell router", "client/src/App.tsx"],
      ["4", "Primary runtime trunks: web, server, mobile, cognitive", "deploy/*"],
      ["0", "Committed automated test files in the repo today", "Jest configured but unused"],
    ],
    badges: ["Module Folders", "Shared Schema", "Test Debt"],
    notes:
      "Treat this as a balanced engineering slide: the foldering is strong, but the test gap and a few oversized screens are real maintainability concerns.",
    sources: ["app", "readme", "schema", "mobilePkg"],
  },
  {
    kicker: "EXPERT REVIEW",
    title: "The strongest engineering quality is breadth anchored to one data model",
    subtitle:
      "From a code review perspective, the best part of the system is that it solves many real maintenance workflows without splintering into disconnected tools.",
    cards: [
      ["Strong platform shape", "Web, mobile, backend and AI surfaces are already separated cleanly enough that the system can scale by adding modules rather than rewriting foundations."],
      ["Shared operational core", "The Prisma model count and route breadth indicate one central maintenance data backbone rather than many isolated micro-apps."],
      ["Feature realism", "Work orders, procurement, manuals, KPIs, inspections, collaboration and energy make the app feel like a real plant operations platform, not a demo shell."],
    ],
    badges: ["Platform Shape", "Shared Core", "Real Workflows"],
    notes:
      "This is the strengths slide. Keep it complimentary but technical: architecture, shared data, and domain breadth are the core positives.",
    sources: ["hub", "server", "schema", "mobileHome", "chat"],
  },
  {
    kicker: "CODE REVIEW",
    title: "The main code review concerns are authorization, migrations and test coverage",
    subtitle:
      "The highest-signal risks are not visual polish. They are permission correctness, deployable schema evolution and the lack of automated regression coverage.",
    cards: [
      ["Finding 1", "The user permission API must keep returning and updating canCreateWorkOrder and canCloseWorkOrder because the manpower UI still depends on those switches."],
      ["Finding 2", "Every new Prisma table needs a checked-in migration because the documented deploy flow relies on prisma migrate deploy and will otherwise fail at runtime."],
      ["Action plan", "Add canary tests, tighten API typing, keep auth universal across modules, and continue splitting the largest screens and route handlers into smaller units."],
    ],
    badges: ["Permissions", "Migrations", "Regression Safety"],
    notes:
      "End with the two explicit blocker findings, then summarize the broader engineering recommendation: protect trust boundaries and reduce regression risk.",
    sources: ["auth", "schema", "server", "app"],
  },
];
...
async function slideCards(presentation, idx) {
  ...
  if (idx === 3 && (await pathExists(MOBILE_ICON_PATH))) {
